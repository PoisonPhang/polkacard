...
const NUM_DRAWS: u8 = 1;
const NUM_CARDS: u16 = 52;
const NUM_PLAYERS: u8 = 8;
...
fn main() {
    ...
    let players: Vec<_> = (0..NUM_PLAYERS).map(|_| {
        Keypair::generate_with(&mut csprng)
    }).collect();

    let draws: Vec<_> = players.iter().map(|player| {
        draws(player, VRF_seed)[0]
    }).collect();

    for draw in draws.clone() {
        let (card, signature) = draw;
    ...
    // reveal cards we must call receive
     for (player, draw) in players.iter().zip(draws.clone()) {
        let pub_key = player.public;
        let sig = draw.1;

        println!("CARD!!! {:?}", recieve(&pub_key, &sig, VRF_seed));
    }

    let winner = draws.iter().map(|draw| {
        draw.0
    }).max();
